...
impl Stabilization {
    ...
    pub fn get_kernel_flags(&self, frame: usize, buffers: &Buffers) -> KernelParamsFlags {
        ...
        // RS direction stays in sensor space — the transform pipeline works in landscape
        // sensor coordinates; input_rotation in the shader handles the final rotation.
        let horizontal_rs = self.compute_params.frame_readout_direction.is_horizontal();
        kernel_flags.set(KernelParamsFlags::HORIZONTAL_RS, horizontal_rs);
